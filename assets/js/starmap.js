...
// ── 星系坐标 → 画布像素 ──
// 背景是一张坐标纸（assets/js/starmap-bg.js 的 GRID_STYLE='coord'），分三级刻度：
// 小坐标 = 1 小格、中坐标 = 5 小格、大坐标 = 25 小格。
// 位置完全交给背景的同一个变换函数计算：先在基准比例尺下吸附到小坐标格点，
// 再整体乘 zoom、加 pan —— 吸附只做一次，之后缩放是"整体缩放"，
// 所以任何比例尺下星系都精确落在小坐标格点上（实测偏差 0px），
// 不会出现"放大时在格点上、缩小后滑到格子中间"的情况。
// 星系只保证在小坐标上是整数格，在中/大坐标上不一定落在交点上（与坐标纸用法一致）。
// canvas 左上角与视口左上角重合（画布铺满内容区），所以视口像素即画布像素。
// 若想让星系落在中坐标格点上，把 starmap-bg.js 的 COORD_SNAP_CELLS 设为 5；
// 设为 25 则落在带标记的大坐标交点上。
